...
def arithmetic(puzzle="SEND+MORE=MONEY", base=10):
    ...
    # letters
    letters = { l: variable(0, base-1) for l in set("".join(problem)) }
    ...
    # constraints
    constraint(alldifferent(letters.values()))
    constraint(sum(words[:-1]) == words[-1])

    for word in expr_pb: constraint(word[0] > 0)

    assert solve(letters.values())
